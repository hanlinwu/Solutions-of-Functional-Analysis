...
\usepackage{ctexcap,geometry,amsmath,listings,tabularx,xcolor,graphicx,multirow,enumerate,amssymb,amscd,extarrows,mathrsfs,rotating,verbatim}
\pagestyle{plain}
\title{《泛函分析选讲》习题参考解答}
...
\end{center}
 ...
 \begin{exercise}
  ...
  \end{exercise}
  \begin{proof}
    (1) $\h B=\h A/J, \|a\|=\inf_{x\in [a]}\|x\|,\forall a\in \h B$. 由商模的定义知$\|[e]\|=\inf_{x\in [e]}\|x\|\leq\|e\|=1$. 又因为$J$是极大理想, 故由命题2.1.13知, $e\notin J$. 故$[e]\neq [\theta]$. 由此及$\|[e]\|=\|[ee]\|=\|[e][e]\|\leq \|[e]\|\cdot\|[e]\|$. 进一步知$\|[e]\|\geq 1$. 故$\|[e]\|=1$.
  ...
  \end{exercise}
  \begin{proof}
    (1) 注意到$(T^*Tx,x)=(Tx,Tx)=\|Tx\|_H^2\geq 0$. 故$T^*T$为正算子. 由此及推论2.5.9知, $\exists P\in \h L(H)$. 使得$P^2=T^*T$, 且$P=P^*$. 故对$\forall x\in H$,
    \[
   ...
   (2) 若$T$可逆, 则$T^*,T^*T$也可逆. 类似(1)可证, $T^*T$为正算子. 故由推论2.5.9知. $T^*T$存在可逆平方根$P$. 令$U:=TP^{-1}$, 则$U$可逆且
   \[
